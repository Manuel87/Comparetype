...
// not actually within the cloud javascript // this was something developed forehand
(function() {
...
// collider rects shapes / intersects
...
// when overlapping
	function overlaps(tree, otherTree, aox, aoy, box, boy) {
		if (rectCollide(tree, otherTree, aox, aoy, box, boy)) {
			alert(tree.children[0].children[0].children[0].children[0].children[0].children[0].children[0].y + " " + tree.children[0].children[0].children[0].children[0].children.length)
			if (tree.children == null) {
				if (otherTree.children == null) return true;
				else for (var i=0, n=otherTree.children.length; i<n; i++) {
					// no clue when this actually happens //
	...
	var w = 960,
			h = 270,
			p = 5; //the less the more detailed?
	...
	//charText and Count (3)
	var glyphs = d3.range(3).map(function(d, i) {
		d += "";
		return {
			text: "x", //d
			code: d.charCodeAt(0),
...
// creating the collision rectangles //
	function sprite(c, text, s, dy, w, h) {
		...
		c.fillStyle = "#000";
		c.textAlign = "center";
		c.font = ~~(s / ratio) + "px TastyRegular";
