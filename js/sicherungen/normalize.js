...
ManusComperator.prototype.Font = function ( fontId ) {
	...
	this.$measureCanvas.width = 1000; //1000
	this.$measureCanvas.height = 2000; //2000
	this.$measureCanvas.id = "canvas_" + fontId;
	//this.$measureCanvas.style.display = "none"; // hide canvas // debugging
...
ManusComperator.prototype.Font.prototype.measure = function () {
	var script = globalVars.activeScript.writingSystem,
	scriptStyle = globalVars.activeScript.scriptStyle,
	fontStyle = globalVars.activeScript.fontStyle || false;
	...
	var myChar = this.myChar = globalVars.writingScripts[script].scriptStyle[scriptStyle].typographicMeasurement.string;
	// should get into the globalVars somewhere earlier
	//var myChar = document.getElementById("typographicsizingcharacter"); // $('#typographicsizingcharacter');
	globalVars.measuredFonts[this.fontName][myChar] = globalVars.measuredFonts[this.fontName][myChar] || false;
	//globalVars.average_measure_em = myChar.getAttribute("data-height") || 0.5; // average Height
	globalVars.average_measure_em = globalVars.writingScripts[script].scriptStyle[scriptStyle].typographicMeasurement.averageheight;
	...
	console.log("globalVars.average_measure_em = " + globalVars.average_measure_em);
	//myChar = this.myChar = globalVars.writingscripts[script].scriptStyle[scriptStyle].typographic_measurement.string || (myChar.innerText || myChar.textContent) || "x";//myChar.text() || "x"; //innerText (< IE 9) // (node.innerText || node.textContent)

	//old (ex-height)
	//this.fontsizeadjust =  globalVars.measure_fontsize / this.$measureXHeight.height(); // old measurement method

	//this.characters[Char].height = Font[Char].height;
	if( globalVars.measuredFonts[this.fontName][myChar]) { //this.canvas_xHeight[this.fontName]) {
	console.log("first")
		this.fontsizeadjust = 1000 * globalVars.average_measure_em /  globalVars.measuredFonts[this.fontName][myChar].height; //this.canvas_xHeight[this.fontName];
	} else {
		console.log("second");
		var CanvasFont = this.MeasureFontOnCanvas(this.fontName, myChar, "block", 1000, fontStyle); //globalVars.measure_fontsize*2should be always 1000 -> look at: globalVars.measure_fontsize
		this.fontsizeadjust = 1000 * globalVars.average_measure_em / globalVars.measuredFonts[this.fontName][myChar].height
		//this.characters[myChar].height;

		//this.fontsizeadjust = globalVars.measure_fontsize / this.characters[myChar].height; //this.canvas_xHeight[this.fontName]; //CanvasFont[myChar].height;
	}	// globalVars.measure_fontsize  is 500, should be 1000;

	//console.log( "this.characters[" + myChar + "].height = " + this.characters[myChar].height);

	console.log( "globalVars.measuredFonts[" + this.fontName + "][" + myChar + "] = " + globalVars.measuredFonts[this.fontName][myChar].height);
	console.log( "this.fontsizeadjust = " + this.fontsizeadjust);
...
ManusComperator.prototype.Font.prototype.style = function () {
	var adjustedfontsize,
		...
		console.log("neFontEm = " + newFontEm);
	...
	$(".chooseFontsContainer .module_" + this.fontId).attr("data-height", this.myChar + "-height: " + globalVars.measuredFonts[this.fontName][this.myChar].height / 1000 + "em"); //this.characters[this.myChar].height
...
ManusComperator.prototype.normalizeFonts = function() {
	...
	RoundFontsize.prototype.update();
...
ManusComperator.prototype.Font.prototype.MeasureFontOnCanvas = function(FontFamily, Char, display, sizeKegel, fontStyle) {
		...
		if (fontStyle === "smallcaps") { Font.variant = "small-caps ";} else { Font.variant = ""; }
		...
		console.log("canvas font = " + ctx.font);
		ctx.fillText(Char, 0, sizeKegel);
		...
		globalVars.measuredFonts[this.fontName][Char].height = Font[Char].height;
		return Font;
	}; // End MeasureFont
